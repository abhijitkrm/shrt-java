...
make >/dev/null 2>&1
DATA_DIR="$DIR" PORT="$PORT" SERVER=mini java -XX:+UseParallelGC -cp classes shrt.Main >/tmp/shrt-java-smoke.log 2>&1 &
SRV=$!
trap 'kill $SRV 2>/dev/null; rm -rf "$DIR"' EXIT

for i in $(seq 1 50); do curl -sf "$BASE/api/health" >/dev/null 2>&1 && break; sleep 0.2; done
...
curl -s -D - -o /dev/null -X OPTIONS "$BASE/api/shorten" | grep -i access-control
